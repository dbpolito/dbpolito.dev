...
<body class="bg-gray-100">
    <div>
        <nav x-data="{ open: false }" @keydown.window.escape="open = false" class="bg-gray-800">
            @php
            $menu = [
            (object) ['route' => 'posts', 'name' => 'Posts'],
            // (object) ['route' => 'uses', 'name' => 'Uses'],
            // (object) ['route' => 'about', 'name' => 'About'],
            (object) ['route' => 'contact', 'name' => 'Contact'],
            ];
            @endphp
            <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex items-center justify-between h-16">
                    <div>
                        <h1>
                            <a href="{{ route('home') }}" class="flex text-white text-lg leading-loose">
                    ...
                    <div class="hidden md:block">
                        <div class="ml-10 flex items-baseline">
                            @foreach ($menu as $item)
                                <a href="{{ route($item->route) }}" class="ml-2 px-3 py-2 rounded-md text-sm font-medium {{ Route::currentRouteNamed($item->route) ? 'text-white bg-gray-900' : 'text-gray-300 hover:text-white hover:bg-gray-700' }} focus:outline-none focus:text-white focus:bg-gray-700">{{ $item->name}}</a>
                            @endforeach

                            <a class="px-3 py-2 rounded-md hover:bg-gray-700" href="#" title="Português">
                            ...
                            <a class="px-3 py-2 rounded-md hover:bg-gray-700" href="#" title="English">
                                <svg class="h-6 w-6 inline-block" viewBox="0 0 512 512">
    ...
    <div :class="{'block': open, 'hidden': !open}" class="hidden md:hidden">
        <div class="px-2 pt-2 pb-3 sm:px-3">
            @foreach ($menu as $item)
            <a href="{{ route($item->route) }}" class="mt-1 block px-3 py-2 rounded-md text-base font-medium {{ Route::currentRouteNamed($item->route) ? 'text-white bg-gray-900' : 'text-gray-300 hover:text-white hover:bg-gray-700' }} focus:outline-none focus:text-white focus:bg-gray-700">{{ $item->name }}</a>
            @endforeach
        </div>
    </div>
</nav>
